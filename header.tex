...
\usepackage{lmodern} % loads Latin Modern fonts, enhanced versions of the Computer Modern fonts. 
	...
	% Also solves rasterized fonts issue on incomplete LaTeX installations without CM-super fonts
...
\newcommand{\const}{\textit{const}} % shorthand for constant. works in math mode with good kerning
...
\renewcommand{\dif}[2][]{\mathop{}\!\mathrm{d}^{#1} #2}
\renewcommand{\Dif}[2][]{\mathop{}\negmedspace\mathrm{D}^{#1} #2}
\newcommand{\pdif}[2][]{\mathop{}\!\partial^{#1} #2}
\newcommand{\ddif}[2][]{\mathop{}\!\delta^{#1} #2}
\newcommand{\Ddif}[2][]{\mathop{}\!\Delta^{#1} #2}
